...
def get_run_id():
	...
	last_row = spark.sql(f"""select * from {control_db}.dim_control order by run_id desc limit 1""")

	if len(last_row.head(1)) == 0:
		run_id = 1
	else:
		run_id = last_row.first()['run_id'] + 1
	return run_id
def get_business_date(param_name):

	business_date = get_param('REPORTING_DATE')
	year = business_date[:4]
	month = business_date[4:6]
	day = business_date[6:8]
	business_date = year + "-" + month + "-" + day
	return business_date

def feed_dim_run(attributes):

	if 'end_count' not in attributes:
		attributes['end_count'] = 'NULL'
		attributes['end_time'] = 'NULL'
	# Added for jurisdiction on 24-06-2021

	spark.sql(f"insert into {control_db}.dim_run values ("+str(attributes["run_id"])+",'"+attributes["process_name"]+"','"+attributes["process_desc"]+"','"+str(attributes["start_time"])+"','"+str(attributes["end_time"])+"',"+str(attributes["start_count"])+","+str(attributes["end_count"])+",'"+attributes["jurisdiction"]+"')")


def feed_dim_control(attributes):

	if attributes['status'] == 'STARTED':
		attributes['end_time'] = 'NULL'
	else:
		attributes['end_time'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
	spark.sql(f"""insert into {control_db}.dim_control values ("""+str(attributes["run_id"])+ ",'"+attributes["business_date"]+"','"+attributes["status"]+"','"+str(attributes["start_time"])+"','"+str(attributes["end_time"])+"')" )

	if attributes['status'] != 'STARTED':
		feed_dim_run(attributes)

def call_engine(run_id, process_rk, process_name, input_table, output_table, executor, jurisdiction,spark,sc):
	#call to CRM ALLOCATION
	if executor=="calculator":
		# Added for jurisdiction on 24-06-2021
		#engine(run_id, process_rk, process_name, input_table, output_table, executor)
		engine(run_id, process_rk, process_name, input_table, output_table,executor,jurisdiction,spark,sc)
	elif executor=="validator":
		query_dict = engine(run_id, process_rk, process_name, input_table, output_table,executor,jurisdiction,spark,sc)
		validate(process_name, query_dict)
	else:
		rwa_logger("rwa-WAR-061", comment = "Invalid execution type entered" )
		raise Exception("Invalid execution type entered")

def get_process_param(process_name, run_id ,jurisdiction):
	"""
	Function ot retrieve attributes from dim_process table as per process_name
	"""
	attributes = dict()

	attributes['process_name'] = process_name
	...
	#Fetch attributes from process table
	process_df = spark.sql(f"select * from {reference_db}.dim_process")
	process_df = process_df.filter(col("process_name") == process_name)
	process_list = process_df.collect()

	if process_list:
		process_dict =  process_list[0].asDict()
		attributes['process_rk'] = process_dict['process_rk']
		attributes['process_desc'] = process_dict['process_desc']
		attributes['input_table'] = process_dict['input_table']
		attributes['output_table'] = process_dict['output_table']
		# timeout in seconds
		attributes['timeout'] = process_dict['timeout_period']
	else:
		rwa_logger("rwa-WAR-001", comment = f"{process_name} doesn't exist in dim_process" )
		raise Exception("Process name doesn't exist")
	# Added for jurisdiction on 24-06-2021
	#attributes['inputquery'] = "select count(*) as count from rtd_shared_trans."+attributes['input_table']
	attributes['inputquery'] = f"select count(*) as count from {calc_input_db}." + attributes['input_table']+" where  " + attributes['jurisdiction'] + "_REGULATOR_FLAG="+''' "Y" '''
	attributes['outputquery'] = f"select count(*) as count from {calc_output_db}."+attributes['output_table']+" where run_id="+str(attributes['run_id'])
...
if __name__ == "__main__":
	...
	try:
		...
		attributes["run_id"] = run_id
		attributes["business_date"] = get_business_date("ABC")
		attributes["status"] = 'STARTED'
		attributes["start_time"] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
		attributes["start_count"] = 0
		attributes["start_count"] = 0
		attributes["process_name"] = 'CRM Allocation STD'
		attributes["process_desc"] = 'CRM Allocation Process - STD'
		attributes["jurisdiction"] = 'pra'
		feed_dim_control(attributes)
	except Exception as msg:
		print(msg)
		rwa_logger("rwa-ERR-112", comment = f"Error {msg} occured while running CRM ALLOCATION")
	try:
		print()
		print("=========================")
