...
yaml_path = cicd_bucket_name + "/rtd/dags/config/basel_controller_dag_util_v1.1.yaml"
...
###############################################################
airflow_dag_name = 'rtd_basel_controller'
root_folder = ''
...
status_output_path = CONFIG_DICT['status_output_path'] + 'rtd_basel_contoller_status' + RUN_ID
...
def final_status(section_list,**kwargs):
    task_dict = {}
    ...
    for task_instance in kwargs['dag_run'].get_task_instances():
        ...
        for section in section_list:
            ...
            for dag_task in procedure_list:
                if dag_task in  task_instance.task_id and task_dict.get(dag_task) !=None and task_instance.current_state() != 'success': #check only those dag status which is in config file
                    print(dag_task)
    ...
    file_path = status_output_path + '/rtd_controller_end_status.json'
...
#final_status = DummyOperator(task_id="Final_status", dag=DAG)
...
final_status = PythonOperator(
    ...
    op_kwargs={'section_list' : section_list},
...
start >> check_schedule >> logger_service >> logger_service_wait >> set_execution
previous = set_execution
